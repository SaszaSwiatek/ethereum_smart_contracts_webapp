...
@user_api_blueprint.route('/api/user/<user_id>/email', methods=['GET'])
def get_email(user_id):
    if current_user.is_authenticated:
        item = User.query.filter_by(id=user_id).first()
        if item is not None:
            data = item.to_json()
            response = jsonify({'message': 'success','result': data['email']})
        else:
            response = jsonify({'message': 'Cannot find email'}), 404

        return response
    else:
        return make_response(jsonify({'message': 'You are not longer logged in'}))

@user_api_blueprint.route('/api/user/<user_id>/eth_prv_key', methods=['GET'])
def get_eth_prv_key(user_id):
    if current_user.is_authenticated:
        item = User.query.filter_by(id=user_id).first()
        if item is not None:
            data = item.to_json()
            response = jsonify({'message': 'success','result': {'eth_prv_key' : data['eth_prv_key'],'eth_address': data['eth_address']}})
        else:
            response = jsonify({'message': 'Cannot find key'}), 404
